...
parser.add_argument('--model-cfg', type=str, default='/home/himansh/SWE/yoloface/cfg/yolov3-face.cfg',
                    help='path to config file')
parser.add_argument('--model-weights', type=str,
                    default='/home/himansh/SWE/yoloface/model-weights/yolov3-wider_16000.weights',
...
def _main():
    ...
    while True:
        ...
        faces, coords = post_process(frame, outs, CONF_THRESHOLD, NMS_THRESHOLD)
        image_obj = Image.open(args.image)
        for i in range(len(coords)):
            new_img = image_obj.crop((coords[i][0], coords[i][1], coords[i][2], coords[i][3]))
            name = output_file.split('.')[0]
            new_img.save('./final_bbox/'+name+'_'+str(i).zfill(3)+'.jpg')
